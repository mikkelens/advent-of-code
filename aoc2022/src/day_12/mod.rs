...
/// Puzzle input is a heightmap, where heights are given by characters "a" to "z",
/// where "z" is the highest.
...
/// 
/// To reach the best location "E", we can move one character/square up, down left or right.
/// We can however only move one up per square, eg. "a" to "b",
/// but we can drop more than one if needed, eg. "g" to "b".
/// 
/// SOLVE: Fewest steps required to move from current position to the location with the best signal
...
impl Position2D {
    fn after_step(&self, dir: &StepDirection) -> Self {
        ...
        Position2D { x: x as usize, y: y as usize }
    }
}
struct Heightmap {
    map: Vec<Vec<CharPosition>>, // 2D map
    starting_pos: Position2D,
    best_location: Position2D
}
