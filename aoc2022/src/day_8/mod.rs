...
impl Runnable for Solution {
    fn run_with_input(&self, input: String) {
        ...
        println!(
            "Score of the place with the highest scenic score: {}",
            part_2_solve(input)
        );
    }
}
...
fn part_1_solve(input: &str) -> usize {
    ...
    let all_rows = (0..grid.height()).flat_map(|y| {
        let left: Box<dyn Iterator<Item = GridCoord>> =
            Box::new((0..grid.width()).map(move |x| GridCoord::from((x, y))));
        let right = Box::new(
            (0..grid.width())
                .rev()
    ...
    let all_columns = (0..grid.width()).flat_map(|x| {
        let top: Box<dyn Iterator<Item = GridCoord>> =
            Box::new((0..grid.height()).map(move |y| GridCoord::from((x, y))));
        let bottom = Box::new(
            (0..grid.height())
                .rev()
...
fn parse_grid(input: &str) -> Grid<Tree> {
    ...
    for (y, line) in input.lines().enumerate() {
        for (x, column) in line.trim().chars().enumerate() {
            ...
            *tree = Tree {
                height: { (column as usize - '0' as usize) as u8 },
            };
        }
    }
    grid
