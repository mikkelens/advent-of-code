...
/// If it did have contradictions or similar, the rules would be circular (chicken or egg).
/// Both X and Y page numbers need to be present in an update for their immediate ordering to
/// have an effect, but this shouldn't make any difference for other values between them.
/// We may assume that updates do not print the same page number more than once.
/// # Solution A:
/// If we can make a way of sorting or dynamically comparing values specified in the rules,
/// the rest is trivial.
/// The problem is the implementing this is not straight-forward,
/// the comparison rules are arbitrary and only specified on a need-to-know basis.
/// It may be possible to use the need-to-know information to construct a truly ordered list,
/// though, that can be used to compare with the updates.
/// This would at least be a lot faster than checking for any specifically illegal rule in the
/// whole list.
/// The only way I can this being done correctly is by collecting all the rules,
/// then starting with the only page number that does not have a rule with itself as the Y value.
/// The problem with this alone is that multiple page numbers can be mentioned only as X.
/// If multiple numbers are mentioned only as X, their relative ordering is still irrelevant.
/// Same goes for only Y, but also for all values that are not compared directly. If 1<5, 3<6,
/// and 1<6, that does not mean 1<3; for all intents and purposes they are equal values.
/// This means there are multiple valid sorting configurations for a set of updates.
