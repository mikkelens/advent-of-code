...
/// Use previous knowledge to create some sufficient sorted set, then apply to
/// updates.
fn solve(input: impl AsRef<str>) -> u32 {
	...
	// assume sorted correctly, we just need to run it against the upgrades and see
	// if the filtered version matches with them. If not, we can use the filtered
	// version to get the right result
	upgrades
		.into_iter()
		.filter_map(|upgrade| {
