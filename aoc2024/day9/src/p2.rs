...
/// # Problem
/// For each file (identified by ID) from the right, try and move it as much left as is possible.
/// What is the filesystem checksum then?
/// # Solution
/// Checksum works the same.
/// Instead of moving by position, lets move whole blocks at once.
/// According to the example, we do not need to check for freed up space during compacting.
...
fn solve_a(mut files: Vec<File>, mut free_spaces: Vec<FreeSpace>) -> u64 {
    ...
    for file in files.iter_mut().rev() {
        ...
        if let Some(leftmost_space) = free_spaces.iter_mut().find(|free_space| {
            ...
            // this is important because other files should be at a different spot if there is
            // more space left
            leftmost_space.index.pos.0 += file.len.0 as usize;
            debug_assert_ne!(
                file.index.pos.0, leftmost_space.index.pos.0,
