...
/// # Definitions
/// A trailhead rating is how many distinct routes from 0 to 9 through the map for this trailhead.
/// Different trailheads all have distinct routes, since they start at distinct 0-height places.
/// # Solution
/// Instead of tallying up how many valid leaf nodes (9) can be reached from each trailhead,
/// join together distinct route and sum up how many there are across all trailheads.
/// Actually, is it even necessary to have them be distinct? Isn't every route inherently distinct?
/// ## Note
/// We want to start collecting data on each journey as we go back up the tree, *not* as we go down.
/// It remains true that branch depth is still capped to 9 or 10.
