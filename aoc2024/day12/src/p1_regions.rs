	...
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		...
		for rel_y in y_span {
			for rel_x in self.x_span() {
				...
				write!(f, "{}", match self.region.positions.contains(&current) {
...
/// It should look like two boxes, one may be larger than the other (in either
/// direction).
