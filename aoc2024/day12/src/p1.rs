...
/// where every flower (ASCII character) connects to another if they are of the
/// same type, and are next to each other cardinally.
/// The sum of each region's area multiplied by its perimeter is the total
/// (result). ## Definitions
/// The area of regions is equal to their amount of flowers (characters).
/// The perimeter is the sum of the edge counts from the outer edge and the
/// inner edge(s). The outside of the input is also an edge, not just between
/// different regions. # Solution
/// Divide into regions of positions, then calculate perimeters & areas to sum
/// them.
