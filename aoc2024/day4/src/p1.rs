...
/// # Constraints
/// The word may appear in any direction, all 8 directions (cardinal *and*
/// diagonal) Input is a rectangle, meaning every line is the same length
/// (amount of characters) # Solution
...
/// If the width of the rectangle is 8, then a center-right iter would skip no
/// characters, but a down-left iter would skip some 7 characters. These
/// iterators are only correct if the next character in them are on the right
/// line: The center-right iter does not yield four characters at the 0+6th
/// place (there are only 0..8 characters in this example). To filter out these
/// cases of each iterator, we can either make sure the iterator fails for the
/// case, or make it never happen. Never making it happen, or never generating
/// it, is possible by starting the iterator with a certain offset or ending it
